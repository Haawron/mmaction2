...
def main():
    ...
    if not p_features.is_dir():
        ...
        subprocess.check_call(f'bash slurm/gcd4da/commons/extract_features_ucf-hmdb_jid.sh {args.jid}', shell=True)
        print('\n\n\ndone.\n\n')
    ...
    global_best_model, ks, score_rows = train_wrapper(Xs, anns, num_known_classes=num_classes, fixed_k=args.fixed_k, n_tries=args.n_tries, p_features=p_features)
    print('done\n')
...
def train_wrapper(Xs, anns, num_known_classes=5, fixed_k:int=None, n_tries=20, p_features=None, verbose=True):
    ...
    if verbose:
        ...
        p_best_csv = p_features.parent / 'sskmeans_best.csv'
        with p_best_csv.open('w') as f:
            writer = csv.writer(f)
            writer.writerow([''])
