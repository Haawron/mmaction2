...
output=$(python slurm/print_best_scores.py -m gcd4da_phase0_lower_way -o --task ${source}_${target} -ig)
...
OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 torchrun --nproc_per_node=4 --master_port=$((10000+$RANDOM%20000)) tools/train.py configs/recognition/hello/ours/gcd4da_phase1_tsm_ek100.py --launcher pytorch \
    --work-dir work_dirs/train_output/ek100/tsm/gcd4da_phase1_lower_way/${task}/${SLURM_ARRAY_JOB_ID}__${SLURM_JOB_NAME}/${SLURM_ARRAY_TASK_ID}/${current_time} \
    --cfg-options \
        ...
        lr_config.warmup_by_epoch=True \
        lr_config.warmup_iters=5 \
        lr_config.warmup_ratio=.1 \
    --validate \
    --test-best --test-last
