...
#SBATCH -J closed_k2b-tsf-cop_pretrain
#SBATCH -p batch
#SBATCH --gres=gpu:4
...
config='configs/recognition/cdar/03_simnreal/021_closed_k400_babel/02_timesformer/011_cop_pretrain/k2b_tsf_cop_pretrain.py'
...
OMP_NUM_THREADS=${N} MKL_NUM_THREADS=${N} torchrun --nproc_per_node="${N}" --master_port=$((10000+RANDOM%20000)) tools/train.py "$config" \
    ...
    --work-dir "$workdir" \
    --cfg-options \
        optimizer.lr="$lr" log_config.interval=50 \
