...
local_indices=(3 4)
idx0=$(( SLURM_ARRAY_TASK_ID % ${#local_indices[@]} ))
seed=$(( SLURM_ARRAY_TASK_ID / ${#local_indices[@]} ))

local_index="${local_indices[idx0]}"
...
add_on='locality'
extra_setting="l1_${local_index}"  # 'default' if none
...
OMP_NUM_THREADS=${N} MKL_NUM_THREADS=${N} torchrun --nproc_per_node="${N}" --master_port=$((10000+RANDOM%20000)) tools/train.py "$config" \
    ...
    --cfg-options \
        ...
        model.forward_kwargs.index._delete_='True' model.forward_kwargs.index.l="$local_index" \
