...
#SBATCH -J closed_k2b-tsm-l2g1_tlahead
#SBATCH -p batch_vll
#SBATCH --gres=gpu:8
...
add_on='locality'
extra_setting='l2g1_45_tlahead'  # 'default' if none
...
config='configs/recognition/cdar/03_simnreal/021_closed_k400_babel/01_tsm/010_source_only/temporal_locality/k2b_tsm_l2g1_tlahead.py'
...
OMP_NUM_THREADS=${N} MKL_NUM_THREADS=${N} torchrun --nproc_per_node="${N}" --master_port=$((10000+RANDOM%20000)) tools/train.py "$config" \
    ...
    --cfg-options \
        ...
        total_epochs=5
# total epochs 늘리면 오히려 떨어지는 것 같음
