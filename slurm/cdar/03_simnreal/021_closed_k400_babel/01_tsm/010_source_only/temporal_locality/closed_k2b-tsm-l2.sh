...
add_on='locality'
extra_setting="l2_34"  # 'default' if none
...
OMP_NUM_THREADS=${N} MKL_NUM_THREADS=${N} torchrun --nproc_per_node="${N}" --master_port=$((10000+RANDOM%20000)) tools/train.py "$config" \
    ...
    --cfg-options \
        ...
        model.forward_kwargs.index._delete_='True' model.forward_kwargs.index.l="[3,4]" \
        data.videos_per_gpu=12 data.workers_per_gpu=10
