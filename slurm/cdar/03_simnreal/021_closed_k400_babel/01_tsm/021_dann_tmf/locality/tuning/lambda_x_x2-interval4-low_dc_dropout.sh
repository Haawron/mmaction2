...
#SBATCH -J closed_k2b-tsm-dann-tmf-dc_glx+cop-lambda_x_x2-interval4-low_dc_dropout
#SBATCH -p batch_grad
#SBATCH --gres=gpu:8
...
extra_setting='dc_glx+cop-lambda_x_x2-interval4-low_dc_dropout'  # 'default' if none
...
OMP_NUM_THREADS=${N} MKL_NUM_THREADS=${N} torchrun --nproc_per_node="${N}" --master_port=$((10000+RANDOM%20000)) tools/train.py "$config" \
    ...
    --cfg-options \
        ...
        model.neck.2.loss_weight=2 data.train.0.pipeline.0.frame_interval=4 \
        model.neck.0.dropout_ratio=.01 model.neck.1.dropout_ratio=.01 model.neck.2.dropout_ratio=.01 \
