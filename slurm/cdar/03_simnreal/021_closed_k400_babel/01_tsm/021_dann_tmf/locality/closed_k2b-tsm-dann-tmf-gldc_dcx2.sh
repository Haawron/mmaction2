...
#SBATCH -J closed_k2b-tsm-dann-tmf-gldc_dcx2
#SBATCH -p batch_vll
#SBATCH --gres=gpu:8
...
add_on='locality'
extra_setting='gldc_dcx2'  # 'default' if none
...
config='configs/recognition/cdar/03_simnreal/021_closed_k400_babel/01_tsm/020_dann/temporal_locality/k2b_tsm_dann_lgdc_dcx2.py'
ckpts=(
...
OMP_NUM_THREADS=${N} MKL_NUM_THREADS=${N} torchrun --nproc_per_node="${N}" --master_port=$((10000+RANDOM%20000)) tools/train.py "$config" \
    ...
    --cfg-options \
        ...
        model.neck.2.temporal_locality='global-local' \
