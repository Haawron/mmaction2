...
#SBATCH -J closed_k2b-tsm-dann_tmf-target_randaug
#SBATCH -p batch
#SBATCH --gres=gpu:4
...
lrs=(8e-3 2e-3 1e-3 5e-4)
lr="${lrs[SLURM_ARRAY_TASK_ID]}"

config='configs/recognition/cdar/03_simnreal/021_closed_k400_babel/01_tsm/021_dann_tmf/k2b_tsm_dann_tmf_target_randaug.py'
...
OMP_NUM_THREADS=${N} MKL_NUM_THREADS=${N} torchrun --nproc_per_node="${N}" --master_port=$((10000+RANDOM%20000)) tools/train.py "$config" \
    ...
    --cfg-options \
        ...
        ckpt_revise_keys='' total_epochs=20 checkpoint_config.interval=5 \
