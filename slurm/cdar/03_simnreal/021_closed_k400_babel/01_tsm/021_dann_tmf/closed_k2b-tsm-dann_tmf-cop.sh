...
#SBATCH -J closed_k2b-tsm-dann_tmf-cop
#SBATCH -p batch_grad
#SBATCH --gres=gpu:8
...
add_on='default'
extra_setting='cop'  # 'default' if none
...
config='configs/recognition/cdar/03_simnreal/021_closed_k400_babel/01_tsm/021_dann_tmf/k2b_tsm_dann_tmf.py'
ckpts=(
...
OMP_NUM_THREADS=${N} MKL_NUM_THREADS=${N} torchrun --nproc_per_node="${N}" --master_port=$((10000+RANDOM%20000)) tools/train.py "$config" \
    ...
    --work-dir "$workdir" \
    --cfg-options \
        optimizer.lr="$calibed_lr" checkpoint_config.interval=5 \
        ckpt_revise_keys='' \
        load_from="$ckpt" \
