...
#SBATCH -J closed_k2b-tsm-dann_tmf-switch_randaug
#SBATCH -p batch
#SBATCH --gres=gpu:4
...
add_on='switch'
extra_setting='target_randaug'  # 'default' if none
...
config='configs/recognition/cdar/03_simnreal/021_closed_k400_babel/01_tsm/021_dann_tmf/k2b_tsm_dann_tmf_source_switch_target_randaug.py'
...
OMP_NUM_THREADS=${N} MKL_NUM_THREADS=${N} torchrun --nproc_per_node="${N}" --master_port=$((10000+RANDOM%20000)) tools/train.py "$config" \
    ...
    --cfg-options \
        ...
        ckpt_revise_keys='' \
        total_epochs=50 evaluation.interval=1 checkpoint_config.interval=10 \
