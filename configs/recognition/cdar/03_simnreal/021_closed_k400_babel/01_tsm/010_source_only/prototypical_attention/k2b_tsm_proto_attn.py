...
model = dict(
    ...
    backbone=dict(
        ...
        shift_div=8),
    neck=dict(
        type='PrototypicalAttention',
        in_channels=2048,
        num_clips=3,
        num_segments=8,
    ),
    cls_head=dict(
        type='TimeSformerHead',  # Neck에서 Temporally aggregating 다 해서 그냥 이거 씀
...
pipelines = dict(
    source=dict(
        ...
        valid=[
            dict(type='SampleFrames', clip_len=8, frame_interval=2, num_clips=3, test_mode=True),
            dict(type='RawFrameDecode'),
            dict(type='Resize', scale=(-1, 256)),
    ...
    target=dict(
        test=[
            dict(type='SampleFrames', clip_len=8, frame_interval=1, num_clips=3, test_mode=True),
            dict(type='RawFrameDecode'),
            dict(type='Resize', scale=(-1, 256)),
            dict(type='CenterCrop', crop_size=224),
            dict(type='Normalize', **img_norm_cfg),
