# dataset settings
dataset_types = dict(
    BABELRawframeDataset=dict(
        type='RawframeDataset',
        filename_tmpl='img_{:05d}.jpg',
        with_offset=True,
        start_index=1,
    ),
    K400RawframeDataset=dict(
        type='RawframeDataset',
        filename_tmpl='img_{:05d}.jpg',
    ),
)
...
dataset_settings = dict(
    source=dict(
        dataset_type='K400RawframeDataset',
        train=dict(
            data_prefix='/local_datasets/kinetics400/rawframes/train',
            ann_file='data/_filelists/k400/processed/filelist_k400_train_closed.txt'),
        test=dict(
            data_prefix='/local_datasets/kinetics400/rawframes/train',
            ann_file='data/_filelists/k400/processed/filelist_k400_train_closed.txt')),
    target=dict(
        dataset_type='BABELRawframeDataset',
        valid=dict(
            data_prefix='/local_datasets/babel',
            ann_file='data/_filelists/babel/processed/filelist_babel_test_merged_open_all.txt'),
        test=dict(
            data_prefix='/local_datasets/babel',
