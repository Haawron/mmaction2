...
class COPRawframeDataset(RawframeDataset):
    """Deprecated: now features will be shuffled in the neck `VCOPN`"""
    def __init__(self,
                 ann_file,
                 pipeline,
                 num_clips=3,
        ...
        super().__init__(
            ...
            with_offset,
            multi_class,
            num_classes or factorial(self.num_clips),
            start_index,
            modality,
            sample_by_class=sample_by_class,
            power=power,
            dynamic_length=dynamic_length)
        # if self.test_mode:  # Runner에서 넣어줌
        #     # for valid and test: BaseDataset.evaluate utillizes labels of filelists
        #     # and COP labels generated by _shuffle_and_relabel are gone after
        #     # forwarded through the model. So we should store them for valid & test.
        #     g = torch.Generator()
        #     g.manual_seed(999)  # Deterministically generating labels
        #     self.predefined_labels = torch.randint(
        #         high=self.num_classes,
        #         size=[len(self)],
        #         generator=g
        #     ).tolist()

    def prepare_train_frames(self, idx):  # 이러면 걍 rawframedataset이랑 똑같은 거 아닌가?, rawframe은 clip 순서 보존하나?
        results = super().prepare_train_frames(idx)
        # results = self._shuffle_and_relabel(results)
        return results

    def prepare_test_frames(self, idx):
        results = super().prepare_test_frames(idx)
        # results = self._shuffle_and_relabel(results, self.predefined_labels[idx])
        return results

    def _shuffle_and_relabel(self, results:dict, label=None) -> dict:
        label = label or np.random.randint(self.num_classes)
